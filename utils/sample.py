...
#
# Copy and paste files below from "anaconda3\Library\bin" to "anaconda3/DLLs"
#
...
SOURCE_WCAM = "http://192.168.43.90:8080/video" #read frames from video streams over Wi-Fi (IP and path of the webcam are needed), e.g. DroidApp
...
class Titrator(object):
	def __init__(self):
		...
		self.Cap = None
		Source = "http://192.168.43.90:8080/video"

		print("Source: ",Source)
		print("SOURCE_WCAM: ",SOURCE_WCAM)

		if ( Source == SOURCE_WCAM ):
			Cam = str(0)
			self.Cap = cv2.VideoCapture(SOURCE_WCAM)
			self.Cap.read()
...
#-------------------------------------------------------------------------------
	...
	def get_burette_readings(self, Frame):
		self.WriteWelcome(Frame)
		(Warning, Volume) = self.GetVolume(Frame)
		Area = self.GetArea(Frame)
